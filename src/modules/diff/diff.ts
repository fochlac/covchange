export function diffReports(original: Core.Report, changed: Core.Report): Core.DiffReport {
	...
	const changedFiles = Object.keys(original.files).filter(name => {
		...
		return altered && (altered.statements !== orig.statements || altered.coveredStatements !== orig.coveredStatements)
	})
...
function diffMetrics(original: Core.Metrics, changed: Core.Metrics): Core.Diff {
	return {
		...
		diff: {
			...
			coveredStatements: Math.floor((changed.coveredStatements - original.coveredStatements) * 10) / 10,
			coveredConditionals: Math.floor((changed.coveredConditionals - original.coveredConditionals) * 10) / 10,
